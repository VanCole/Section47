...
#include "S47BTDecorator_CompareValue.h"
...
//UOMFBTDecorator_CompareValue::UOMFBTDecorator_CompareValue(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
//{
//	NodeName = "Blackboard Based Condition";
//	NotifyObserver = EBTBlackboardRestart::ResultChange;
//}

bool US47BTDecorator_CompareValue::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
...
bool US47BTDecorator_CompareValue::EvaluateOnBlackboard(const UBlackboardComponent& BlackboardComp) const
{
	bool bResult = false;
...
EBlackboardNotificationResult US47BTDecorator_CompareValue::OnBlackboardKeyValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
void US47BTDecorator_CompareValue::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
FString US47BTDecorator_CompareValue::GetStaticDescription() const
{
...
void US47BTDecorator_CompareValue::BuildDescription()
{
...
void US47BTDecorator_CompareValue::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(US47BTDecorator_CompareValue, BlackboardKey.SelectedKeyName))
	{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(US47BTDecorator_CompareValue, BasicOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(US47BTDecorator_CompareValue, ArithmeticOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(US47BTDecorator_CompareValue, TextOperation))
	{
...
void US47BTDecorator_CompareValue::InitializeFromAsset(UBehaviorTree& Asset)
{
